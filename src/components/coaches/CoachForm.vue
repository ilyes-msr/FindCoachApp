<template>
  <p class="invalid" v-if="!formIsValid">There are some erros, fix them to ba able to register</p>
  <form @submit.prevent="submitForm">
    <div class="form-control" :class="{invalid: !firstName.isValid}">
      <label for="firstname">Firstname</label>
      <input type="text" id="firstname" v-model.trim="firstName.val" @blur="clearValidity('firstName')">
      <p v-if="!firstName.isValid">Field cannot be empty</p>
    </div>

    <div class="form-control" :class="{invalid: !lastName.isValid}">
      <label for="lastname">Lastname</label>
      <input type="text" id="lastname" v-model.trim="lastName.val" @blur="clearValidity('lastName')">
      <p v-if="!lastName.isValid">Field cannot be empty</p>
    </div>

    <div class="form-control" :class="{invalid: !description.isValid}">
      <label for="description">Description</label>
      <textarea name="description" id="description" v-model.trim="description.val" @blur="clearValidity('description')"></textarea>
      <p v-if="!description.isValid">Field cannot be empty</p>
    ...
    <div class="form-control" :class="{invalid: !rate.isValid}">
      ...
      <p v-if="!rate.isValid">Field cannot be empty or negative</p>
    ...
    <div class="form-control" :class="{invalid: !areas.isValid}">
      ...
      <p v-if="!areas.isValid">Field cannot be empty</p>
...
p.invalid {
  background-color: rgba(249, 44, 44, .33);
  padding: .99rem .33rem;
  border-radius: .25rem;
  text-align: center;
}
